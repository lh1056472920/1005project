            ...
            <div v-if="isNum==0" class="xianshi">
                <div v-if="isShow==1&&(item.status==-1||item.status==2)" class="mainsale" v-for="(item,i) in historybuy" :key="i">
                    <div :class="item.type">
                        ...
                        <span v-if="item.type=='sell'||item.type=='sell_market'" class="two">卖出</span>
                        <span v-if="item.type=='buy'||item.type=='buy_market'" class="two">买入</span>
                    </div>
                    <div class="mainBuyContent">
                        ...
                        <div class="one">
                            <p>委托价</p>
                            <p v-if="item.type==='buy'||item.type==='sell'">{{Number(item.price).toString().substring(0,9)}}</p>
                            <p v-if="item.type==='buy_market'||item.type==='sell_market'">市价</p>
                        </div>
                        <div class="one">
                            <p>成交均价</p>
                            <p v-if="item.deal_amount!=0">{{(Number(item['avg_price'])).toString().substring(0,9)}}</p>
                            <p v-if="item.deal_amount==0">0</p>
                        </div>
                    </div>
                    <div class="mainBuyfooter">
                        <span>{{timestampToTime(item['create_date'])}}
                        </span>
                        <div v-if="item.status==-1" :class="item.type">已撤销</div>
                        <div v-if="item.status==2" :class="item.type">已成交</div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
          cbunse(res) {
              this.$toStatistic("cbDealbuyokexhistroy", "查看okex交易记录");
                            ...
                            orderId.forEach(element => {
                                remote
                                    .selectinfoOrderIdoekx({
                                        apikey: urljm(this.Okexkey.apikey),
                                        secret: urljm(this.Okexkey.secretkey),
                                        symbol: this.symbol,
        ...
        getzb() {
            ...
            let url = `http://chaobi.lian2345.com/api/chaobi/deal/oken/order_history/${
                ...
                urljm(this.Okexkey.secretkey)
            }&current_page=1&page_length=200&status=1&symbol=${this.symbol}`;
