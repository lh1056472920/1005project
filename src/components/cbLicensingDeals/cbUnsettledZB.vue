    ...
    <div class="cbUnsettledZB">
        ...
        <main>
            <div v-if="isNum==0" class="xianshi">
                <div v-if="isShow==1&&(item.status==0||item.status==3)" class="mainsale" v-for="(item,i) in cbUnsettledZB" :key="i">
                    ...
                    <div class="mainBuyfooter">
                        ...
                        <div :class="item.type==0?'sell':'buy'" @click="Undo(item.id)">撤销</div>
                    </div>
                </div>
            ...
            <div v-if="isNum==1" class="xianshi">
                <div v-if="isShow1==1" class="mainsale" v-for="(item,i) in cbUnsettledBian1" :key="i">
                    ...
                    <div class="mainBuyContent">
                        ...
                        <div class="one">
                            <p  v-if="item.other=='sell'">最低卖出价</p>
                            <p  v-if="item.other=='buy'">最高买入价</p>
...
export default {
    ...
    methods: {
        cbunse(res) {
              this.$toStatistic("cbDealbuyznsettle", "查看zb未成交交易记录");
        ...
        Undo(data) {
            ...
            var id = data;
            // console.log(data);
            let url = `http://chaobi.lian2345.com/api/chaobi/deal/zb/cancelOrder/${
                this.ZBkey.num
            }?apikey=${this.ZBkey.apikey}&secret=${
                this.ZBkey.secretkey
            }&method=cancelOrder&id=${id}&symbol=${this.symbol}`;

            axios.post(url).then(res => {
                let list = JSON.parse(res.data.list);
                if (list.code && list.code == 3000) {
