<template lang="html">
    <div class="page">
        <HeaderItem title="购买套利模式"></HeaderItem>
        <div class="main">
            <p class="title">
                <img src="@/images/about/vip_light.png" class="title-image" alt="">
                按次数购买（设置一个交易所的一个币种为一次，每次24小时）
            </p>
            <div class="select-panel" v-for="(item,index) in amountList"  :key="index" :class="{selected: active === index,recommend: index === recommend}" v-tap="{methods: changeSelect, active: index}">
                <div class="panel-container">
                    <p class="CNY">¥{{item.CNY}}/{{item.date === 1?'':item.date}}次</p>
        ...
        <div class="bottom">
            <div class="left" v-if="!checkBCH">
                ...
                <p class="remind">您已选择{{selectAmount.date}}次套餐</p> 
            </div>
    ...
    methods: {
        ...
        async submit() {
            ...
            let urlInfo = await getPayURL({
                ...
                times: String(0),
                arbitrage: String(this.selectAmount.date),
